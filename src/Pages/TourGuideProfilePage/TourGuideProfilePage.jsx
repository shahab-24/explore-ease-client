...
const TourGuideProfilePage = () => {
        ...
        useEffect(() => {
                axios.get(`http://localhost:8000/tourGuidesProfile/${id}`)
                .then((res) => {
                        setGuideProfile(res.data)
                })
        }, [id])
        return (
                <div className="min-h-screen bg-gradient-to-br from-white to-sky-100 dark:from-gray-800 dark:to-gray-900 p-6">
                  <motion.div
                  ...
                  >
                    <div className="flex flex-col md:flex-row items-center gap-6">
                      ...
                      />
                      <div>
                        <h2 className="text-2xl font-bold text-gray-800 dark:text-white mb-1">{guideProfile?.name}</h2>
                        <p className="text-sm text-gray-600 dark:text-gray-300 mb-2 italic">
                          {guideProfile?.specialty}
                        </p>
                        <p className="text-gray-700 dark:text-gray-200">
                          <strong>Email:</strong> {guideProfile?.email}<br />
                          <strong>Phone:</strong> {guideProfile?.phone}<br />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {guideProfile?.stories.map((story) => (
                        ...
                        >
                          ...
                          />
                          <div className="p-4">
                            <h4 className="text-lg font-semibold text-gray-800 dark:text-white mb-1">{story?.title}</h4>
                            <p className="text-sm text-gray-600 dark:text-gray-300">{story?.snippet}</p>
                          </div>
                        </motion.div>
                      ))}
                    </div>
